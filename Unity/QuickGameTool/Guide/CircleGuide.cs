...
public class CircleGuide : MonoBehaviour
{
    ...
    public Canvas canvas
    {
        get
        {
            return UIManager.Instance.CurrSceneMainCanvas;
        }
    }
    //指定相机
    [HideInInspector]
    ...
    {
        if (imgHand == null) return;
        if (camera)
        {
            imgHand.position = camera.WorldToScreenPoint(target.position);
        }
        else
        {
            imgHand.position = target.position;
        }
    }
    Vector2 center;
    public void Refresh()
    {
        ...
        float y = _corners[0].y + ((_corners[1].y - _corners[0].y) / 2f);
        Vector3 centerWorld = new Vector3(x, y, 0);
        center = WorldToCanvasPos(canvas, centerWorld);
        //设置遮罩材料中的圆心变量
        Vector4 centerMat = new Vector4(center.x, center.y, 0, 0);
        _material = GetComponent<Image>().material;
        _material.SetVector("_Center", centerMat);
